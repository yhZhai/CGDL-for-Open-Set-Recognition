...
parser.add_argument(
    "--threshold", type=float, default=0.5, help="threshold of gaussian model"
)
...
class GAU(object):
    ...
    def train(self):
        ...
        for j in range(trainsize):
            for i in range(labelnum):
                if traintar[j] == i:
                    locals()["matrix" + str(i)] = np.append(
                        (locals()["matrix" + str(i)]),
                        np.array([np.array(trainfea[j])]),
                        axis=0,
                    )

        for i in range(labelnum):
            locals()["mu" + str(i)] = np.mean(
                np.array(locals()["matrix" + str(i)]), axis=0
            )
            locals()["sigma" + str(i)] = np.cov(
                np.array((locals()["matrix" + str(i)] - locals()["mu" + str(i)])).T
            )
            locals()["gau" + str(i)] = [
                locals()["mu" + str(i)],
                locals()["sigma" + str(i)],
    ...
    def test(self, testsetlist, threshold=args.threshold):
        ...
        def multivariateGaussian(vector, mu, sigma):
            ...
            d = (
                (np.mat(vector - mu))
                * np.mat(np.linalg.pinv(sigma))
                * (np.mat(vector - mu).T)
            )
            p = np.exp(d * (-0.5)) / (
                ((2 * np.pi) ** int(dim / 2)) * (np.linalg.det(sigma)) ** (0.5)
            )
            p = float(p)
            return p
        ...
        for i in range(labelnum + 1):
            locals()["tp" + str(i)] = np.sum(
                (testtar[(locals()["resultIndex" + str(i)])]) == i
            )
            locals()["fp" + str(i)] = np.sum(
                (testtar[(locals()["resultIndex" + str(i)])]) != i
            )
            locals()["fn" + str(i)] = np.sum(
                (result[locals()["targetIndex" + str(i)]]) != i
            )
        ...
        for i in range(labelnum + 1):
            locals()["precision" + str(i)] = locals()["tp" + str(i)] / (
                locals()["tp" + str(i)] + locals()["fp" + str(i)] + 1
            )
            locals()["recall" + str(i)] = locals()["tp" + str(i)] / (
                locals()["tp" + str(i)] + locals()["fn" + str(i)] + 1
            )
            locals()["f-measure" + str(i)] = (
                2
                * locals()["precision" + str(i)]
                * locals()["recall" + str(i)]
                / (locals()["precision" + str(i)] + locals()["recall" + str(i)])
            )
        ...
        mafmeasure = (
            ...
            * performancesum[3]
            * performancesum[4]
            / (performancesum[3] + performancesum[4])
        )
...
if __name__ == "__main__":
    ...
    for epoch in range(1):
        ...
        gau = GAU(epoch)
        omn = [
            "lvae%d/omn_fea.txt" % args.lamda,
            "lvae%d/omn_tar.txt" % args.lamda,
            "lvae%d/omn_pre.txt" % args.lamda,
        ]
        mnist_noise = [
            "lvae%d/mnist_noise_fea.txt" % args.lamda,
            "lvae%d/mnist_noise_tar.txt" % args.lamda,
            "lvae%d/mnist_noise_pre.txt" % args.lamda,
        ]
        noise = [
            "lvae%d/noise_fea.txt" % args.lamda,
            "lvae%d/noise_tar.txt" % args.lamda,
